...
class SendCommand:
    ...
    def handle(update: Update, context: CallbackContext) -> int:
        ...
        if selected == "send":
            ...
            pattern = [[InlineKeyboardButton(text=Messages.MessageMenu.REJECT, callback_data=f"send reject -1 -1"),
                        InlineKeyboardButton(text=Messages.MessageMenu.SEND_TO_CHANNEL, callback_data=f"send send -1 -1")]]
            markup = InlineKeyboardMarkup(pattern)
