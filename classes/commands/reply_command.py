...
class ReplyCommand:
    ...
    def handle(update: Update, context: CallbackContext) -> int:
        ...
        if selected == "send":
            ...
            pattern = [[InlineKeyboardButton(text=Messages.MessageMenu.REJECT, callback_data=f"reply reject -1 {reply_id}"),
                        InlineKeyboardButton(text=Messages.MessageMenu.SEND_TO_CHANNEL, callback_data=f"reply send -1 {reply_id}")],
                       [InlineKeyboardButton(text=Messages.MessageMenu.REPLYED_MESSAGE, callback_data="reply link -1 {reply_id}",
